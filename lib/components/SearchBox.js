...
SearchBox.prototype.render = function () {
  var self = this;
  var loader = new Loader({ loading: this.state.loadingResults });
  ...
  el.setAttribute('class', 'search-box');
  el.appendChild(loader.render());
  el.appendChild(form.render());

  // Add Search Results
  var searchResultsEl = document.createElement('section');
  searchResultsEl.setAttribute('class', 'search-results');
  searchResultsEl.appendChild(results.render());
  el.appendChild(searchResultsEl);
...
SearchBox.prototype._buildSearchResults = function () {
  ...
  if (Object.keys(results).length !== 0 && results.constructor === Object) {
    Object.keys(results).sort().forEach(function (travelDate, _index) {
      var values = results[travelDate]
        .sort(self.state.resultsSort)
        .map(function (value) {
          return new SearchResult(value);
        });
      tabs[travelDate] = new Paginator({
        handleClick: self._handleResultsPageIndexClick.bind(self),
        index: self.state.resultsPageIndex,
        content: values
      });
      // tabs[travelDate].props.handleClick = self._handleResultsPageIndexClick.bind(self);
    });
    console.log(tabs)
    console.log(self.state)
    return new Tabs({ content: tabs, defaultOpenIndex: BLANK_DAY_OFFSET });
...
SearchBox.prototype._handleResultsPageIndexClick = function (index, callback) {
  var self = this;
  console.log(index);
  self.setState({
    hasUpdatedResults: true,

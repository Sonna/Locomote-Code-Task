...
Tabs.prototype.render = function () {
  ...
  Object.keys(this.props.content).sort().forEach(function (key, index) {
    ...
    // values = (typeof values === 'string') ? Array(values) : values;
    ...
    tabContentEl.setAttribute('id', key)
    tabContentEl.setAttribute('class', 'tabcontent animate-bottom')
    if (index === self.props.defaultOpenIndex) {
      // tabsContentEl.lastChild.style.display = 'block';
      // tabsContentEl.lastChild.className += ' active';
      tabContentEl.style.display = 'block';
      tabContentEl.className += ' active';
    }
    tabsContentEl.appendChild(tabContentEl)
